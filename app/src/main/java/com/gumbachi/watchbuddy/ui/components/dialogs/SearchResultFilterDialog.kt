...
fun SearchFilterDialog(
    ...
    onDefaultCheckedChange: (Boolean) -> Unit,
    onSubmit: () -> Unit,
    onCancel: () -> Unit,
    ...
    WatchBuddyDialog(
                    ...
                    leadingIcon = {
                        if (selected) {
                            Icon(
                                ...
                                contentDescription = "${api.name} Shown"
                            )
                        } else {
                            Icon(
        ...
        FlowRow(
            ...
            MediaType.values().forEach { type ->
                val selected = filter.includes(mediaType = type)
                FilterChip(
                    selected = selected,
                    onClick = {
                        when (selected) {
                            true -> onFilterChange(filter - type)
                            false -> onFilterChange(filter + type)
                        }
                    },
                    leadingIcon = {
                        if (selected) {
                            Icon(
                                imageVector = Icons.Filled.Check,
                                contentDescription = "${type.name} Allowed"
                            )
                        }
                    },
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            Checkbox(
                ...
                onCheckedChange = { onDefaultCheckedChange(it) }
            )
